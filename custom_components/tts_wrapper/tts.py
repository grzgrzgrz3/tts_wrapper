...
async def async_get_engine(hass, config, discovery_info=None):
    ...
    tmp_dir = await hass.async_add_executor_job(
        _init_tts_tmp_dir, hass, config[CONF_TMP_DIR]
    )
...
class TTSWrapper(Provider):
    ...
    def get_tts_audio(self, message, language, options=None):
        _LOGGER.warning("proxing call to : {}".format(self._provider))
        res = self._provider.get_tts_audio(message, language, options)
        return res
...
async def _get_provider(hass, provider_name, discovery_info):
    ...
    platform = await async_prepare_setup_platform(
        hass, component_config, DOMAIN, provider_name
    )
    if platform is None:
    ...
    if hasattr(platform, "async_get_engine"):
        return await platform.async_get_engine(
            hass, platform_conf, discovery_info
        )
    else:
